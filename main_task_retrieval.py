...
import random
import os
from metrics import compute_metrics, tensor_text_to_video_metrics, tensor_video_to_text_sim, compute_dsl_metrics
import time
import argparse
...
def _run_on_single_gpu(model, batch_list_t, batch_list_v, batch_sequence_output_list, batch_visual_output_list):
    ...
    for idx1, b1 in enumerate(batch_list_t):
        ...
        for idx2, b2 in enumerate(batch_list_v):
            ...
            # b1b2_logits = model.get_similarity_logits(sequence_output, visual_output, input_mask, video_mask, loose_type=model.loose_type)
            b1b2_logits = model.get_final_similarity(sequence_output, visual_output, input_mask, video_mask, loose_type=model.loose_type)
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    logger.info('\t>>>  R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - R@50: {:.1f} - Median R: {:.1f} - Mean R: {:.1f}'.
                format(dsl_tv_metrics['R1'], dsl_tv_metrics['R5'], dsl_tv_metrics['R10'], dsl_tv_metrics['R50'], dsl_tv_metrics['MR'], dsl_tv_metrics['MeanR']))
    logger.info("DSL Video-to-Text:")
    logger.info('\t>>>  V2T$R@1: {:.1f} - V2T$R@5: {:.1f} - V2T$R@10: {:.1f} - V2T$R@50: {:.1f} - V2T$Median R: {:.1f} - V2T$Mean R: {:.1f}'.
                format(dsl_vt_metrics['R1'], dsl_vt_metrics['R5'], dsl_vt_metrics['R10'], dsl_vt_metrics['R50'], dsl_vt_metrics['MR'], dsl_vt_metrics['MeanR']))
